...
DISP="${DISPLAY:-:99}"
DISP_NUM="${DISP#:}"  # strip leading colon → "99" or "100"
...
# Wait for Xvfb to be ready (check lock file — no dependency on xdpyinfo)
for i in $(seq 1 30); do
    if [ -f "/tmp/.X${DISP_NUM}-lock" ]; then
        # Lock file exists → Xvfb is running. Verify it accepts connections.
...
# Write a permissive system config so root can connect
